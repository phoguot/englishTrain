...
- `module/<Ten>/AGENTS.md` — luật riêng của module.
...
2. **Mapper** `src/Model/<Entity>/<Entity>Mapper.php` — toàn bộ SQL nằm ở đây (`search`, `get`,
   ...
   Không FK constraint — cột liên kết chỉ giao tiếp qua index (`.Codex/rules/02-database.md`).
...
- `module/<Ten>/AGENTS.md` — nếu có luật/quy ước riêng mới.
...
1. Cấu trúc: `module/<Ten>/{config/module.config.php, src/{Controller,Service,Model,Filter}, view/<ten-thuong>}`
   (chi tiết trong `docs/code-standards/crud-convention.md`) + `Module.php` + `AGENTS.md`
   (theo mẫu module có sẵn, ví dụ `module/Attendance/AGENTS.md`).
...
4. Thêm mục vào `docs/03-modules.md` và bảng module trong `AGENTS.md` gốc.
